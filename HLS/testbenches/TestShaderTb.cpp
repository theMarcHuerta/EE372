...
CCS_MAIN(int argc, char** argv) {
    ...
    params_in_obj.num_quads = cpp_quads.size();
    ...
    for (int i = 0; i < cpp_records.size(); i++){
        for (int quad_num = 0; quad_num < cpp_quads.size(); quad_num++){
            quad_in.write(HLS_quads[quad_num]);
        }
        ...
        shade.run(params_in, quad_in, ray_in, attenuation_chan_in, accumalated_color_chan_in, 
        ...
        HLS_bounced_rays.push_back(ray_out.read());
        // if (i == 3589) break;
    }
    ...
    for (int i = 0; i < tot_intersection_tests; i++){
        ...
        if (!(dir_close && col_close && att_close)){
            mismatches++;
            ...
            cout << "Ray CPP dir in: " << tmp_ccc_ray.dir.e[0] << " " << tmp_ccc_ray.dir.e[1] << " " << tmp_ccc_ray.dir.e[2] << endl ;
            ...
            cout << "Ray HLS atte out: " << tmp_att_x << " " << tmp_att_y << " " << tmp_att_z << endl;
            cout << "Ray CPP atten out: " << atten_cpp.e[0] << " " << atten_cpp.e[1] << " " << atten_cpp.e[2] << endl << endl;
