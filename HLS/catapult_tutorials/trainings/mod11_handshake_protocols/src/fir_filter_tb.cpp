...
//============================================================================
// Name        : fir_filter_tb.cpp
// Author      : Jim Corona
// Version     : 2017.06.30
// Copyright   : (c)JimCoronaPE
// Description : Interfacing with Catapult
//============================================================================
...
CCS_MAIN(int argc, char **argv)
{
  ...
  static ac_channel<X_TYPE>  x_chan;
  static ac_channel<Y_TYPE>  y_chan;
  ...
  cout << __FILE__ << "," << __LINE__ << " - Read in " << samples.size() << " input samples." << endl;

  // Loop through samples, calling the fir_filter function for each sample:
  for (vector<STIMULUS_TYPE>::iterator it = samples.begin(); it != samples.end(); ++it) {
    x_chan.write((*it).i_sample);
    //CCS_DESIGN(fir_filter) ((*it).i_sample, filterCoeffs, (*it).o_sample);
  }

  CCS_DESIGN(fir_filter)(x_chan, filterCoeffs, y_chan);

  for (vector<STIMULUS_TYPE>::iterator it = samples.begin(); it != samples.end(); ++it) {
    (*it).i_sample = y_chan.read();
  }

  WriteCSV_Samples("filter_output.csv", samples);
