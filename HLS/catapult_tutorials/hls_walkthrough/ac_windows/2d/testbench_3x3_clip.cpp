...
//     This specific version tests the Sobel with a 3x3 kernel.
//     Input data is 8 bit signed integer.
...
CCS_MAIN(int argc, char *argv[])
{
  ...
  enum {
    ...
    IMG_H        = 200,
    K_R          = 3,         // These enums configure the Kernel size
    K_C          = 3,
  ...
  sobel_orig<ref_dType,ref_accuType,IMG_W,IMG_H,K_R,K_C,BMODE_CLIP>    orig_INST;  // reference model

  // Sobel kernel data
  const ref_kType ref_cx[K_R][K_C]    = { -1,  0,  1, 
                                          -2,  0,  2, 
                                          -1,  0,  1};
  const ref_kType ref_cy[K_R][K_C]    = {  1,  2,  1,
                                           0,  0,  0,
                                          -1, -2, -1};
  ...
  typedef ac_int<8,true>   dType;      // Data type used for this version of the design
  ...
  sobel<dType,accu,kType,IMG_W,IMG_H,K_R,K_C,AC_CLIP>         dut_INST;   // DUT has bit-accurate types
