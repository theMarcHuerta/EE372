...
class camera {
  public:
    ...
    // void render_segment(const hittable& world, int start_line, int end_line, std::vector<std::string>& output) {
    ...
    //         }
    //         lines_remaining -= 1;
    //         output[j] = local_stream.str();
    //         local_stream.str(""); // Clear the stream for the next line
    //         local_stream.clear();
    //     }
    // }
    ...
    void render(const hittable& world) {
        ...
        TaskQueue tasks;
        std::vector<std::string> output(image_height);
        ...
        // Worker threads creation and management
        std::vector<std::thread> workers;
        for (int i = 0; i < num_threads; ++i) {
            workers.emplace_back([this, &tasks, &rows_completed]() {
                while (true) {
                    auto task = tasks.pop();
                    if (!task) break; // Exit loop if no task and finished adding tasks
                    task();
                }
            });
        }

        // After adding all tasks to the queue
        tasks.finish();
