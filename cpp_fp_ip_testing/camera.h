...
class camera {
                ...
                for (int j = start_line; j < end_line; ++j) {
                    for (int i = 0; i < image_width; ++i) {
                        ...
                        for (int sample = 0; sample < samples_per_pixel; ++sample) {
                            ...
                            // }
                            // if (sample == 0 && j == 165 && i==240){
                            //     std::clog <<  r.direction()<< " RAY direction\n";
                            //     std::clog << r.origin() << " RAY ORIGIN\n";
                            // }
                            // std::clog << ray_color(r, max_depth, world) << "\n";
                        }
                        // if (j == 165 && i==240){
                        //     std::clog <<  pixel_color << ": PIXEL COLOR \n";
                        // }
  ...
  private:
    ...
    void initialize() {
        ...
        auto viewport_height = 2 * h * focal_length.toDouble();  // Total viewport height.
        ...
        auto viewport_upper_left = center - (focal_length.toDouble() * w) - viewport_u/2 - viewport_v/2;

        viewport_upper_left.e[2] = FixedPoint(0);

        pixel00_loc = viewport_upper_left + 0.5 * (pixel_delta_u + pixel_delta_v);
        std::clog << lookfrom << "\n";
        std::clog << lookat << "\n";
        std::clog << viewport_upper_left << "\n";
        std::clog << image_height << "\n";
        std::clog << center << "\n";
        std::clog << pixel00_loc << "\n";
        std::clog << pixel_delta_u << "\n";
        std::clog << pixel_delta_v << "\n";
        std::clog << u << "\n";
        std::clog << v << "\n";
        std::clog << w << "\n";
        std::clog << aspect_ratio << "\n";
        std::clog << image_width << "\n";
        std::clog << samples_per_pixel << "\n";
        std::clog << max_depth << "\n";
        std::clog << background << "\n";
    ...
    color ray_color(const ray& r, int max_depth, const hittable& world) const {
        ...
        for(int depth = 0; depth < max_depth; ++depth) {
            ...
            if (!world.hit(current_ray, interval(0.001, 2048), rec)) {
                ...
                //std::clog << "missed world" << "\n";
                // if (depth==0){
                // std::clog << "\n hit rec info:" << "\n\n";
                // std::clog << rec.p << "\n";
                // std::clog << rec.normal << "\n";
                // std::clog << rec.t.toDouble() << "\n";
                // std::clog << rec.u.toDouble() << "\n";
                // std::clog << rec.v.toDouble() << "\n";
                // std::clog << rec.front_face << "\n\n";
                // }
                break;
            }
        ...
        //if (accumulated_color.x().toDouble() > 0.1)          std::clog << accumulated_color << "\n";
